...
### Added
- **VS Code extension: debugger (Stage 1)** — breakpoints, step over/into/out, and locals/globals/call-stack inspection via VS Code's built-in Run-and-Debug UI (`F5`). New `cmd/pipe-dap` binary (standard library only) implements the Debug Adapter Protocol over the same Content-Length-framed stdio transport as `cmd/pipe-lsp` (now shared via new `pkg/rpcframing`, extracted from `cmd/pipe-lsp/main.go` with no behavior change). New `pkg/vm.DebugSession` drives pause/resume/step via a channel-based cooperative hook (`VM.decodeAndCheck`, shared by `Run()` and `executeFrame()`'s previously-duplicated fetch/decode loops) and new exported introspection (`AttachDebugSession`, `StackFrames`, `Locals`, `Globals`). Compile-time symbol names (`CompiledFunction.Name`/`LocalNames`, `Bytecode.GlobalNames`) now survive into the `.pipec` cache and runtime (`CacheVersion` bumped 8→9) so the debugger can show real variable/function names. Scope: a single top-level VM per debug launch — `spawn`-ed code runs un-paused (documented limitation, not a bug; concurrency-aware debugging is a future stage). New `vscode/src/dapPath.ts`/`debugAdapter.ts`, `pipe.dapPath` setting, `make dap` build target. Protocol layer covered by a Go test harness driving raw DAP JSON over `io.Pipe()`; the actual VS Code `F5` experience has not been manually verified (built headless, no VS Code instance available). See [VS Code Extension §Debugger](docs/en/15-vscode-extension.md#debugger).
- **`redact`** — PII masking. AI-assisted by default (catches free-text PII like names and addresses, same as every other AI builtin); pass `{offline: true}` for a zero-network regex-only mode covering structured PII (emails, phone numbers, IPv4, SSNs, credit-card-shaped digit runs). See [AI Builtins §19.3](docs/en/19-ai-builtins.md).
- **`rerank`** — scores and sorts candidates by relevance to a query, completing the `embed` → `nearest` → `rerank` retrieval pipeline. Provider-agnostic (a prompted batch relevance-scoring call, since none of the 6 supported providers expose a dedicated reranker endpoint), reusing the existing `ai_batch`/`ai_parallel` infrastructure for large candidate lists.
- **`moderate`** — fixed-taxonomy content-safety classification (violence, self-harm, hate, sexual, harassment), distinct from `classify`'s user-defined categories. Uses OpenAI's native `/v1/moderations` endpoint when that's the active provider (cheaper, faster, more reliable), otherwise a prompted-JSON fallback against the general taxonomy.
- **`pkg/docexamples`**: a doc-example verification harness — parses every ```pipe code block across `docs/en` and `docs/de` (970 total) and executes the ones that don't need an AI key, the network, stdin, a subprocess, or the filesystem (69 executed, the rest correctly skipped). Runs as part of `go test ./...`. Building it surfaced most of the doc fixes below.
- **`FuzzClosureScoping` / `FuzzRuntimeErrors`** (`pkg/eval`): generative differential fuzzers, distinct from the existing lexer/parser/compiler `FuzzXxx` tests (which only check "does not panic" on arbitrary byte soup) — these build syntactically valid, semantically varied Pipe programs by construction (nested functions mixing shadow-mutate assignments and self-recursive definitions; division/modulo by zero, out-of-bounds indexing, type mismatches, with and without `try`/`catch`) and assert the tree-walker and VM produce byte-identical output. ~2.5M generated programs run clean across this session; `FuzzRuntimeErrors` found the operator-symbol message bug below within a second.

### Fixed
- **Compiler: plain variable/function declarations tracked the wrong source line** — `x: 1` or `fn greet name` recorded line 0 (or leaked the line of whatever was compiled just before them) instead of their real line, because `currentLine` only updates from AST nodes implementing `Pos()`, and neither `VarStatement` nor `FnStatement` did (their own literal/infix right-hand sides often don't either). Found building the debugger's breakpoint tests, where a breakpoint on a plain assignment simply never fired. Fixed by giving both statement types a `Pos()` that delegates to their `Name` identifier, which always carries a real line/column.
- **`pkg/vm.DebugSession`: a breakpoint or step-over could refire when a call returned to its own call-site line** — line-change detection used one global "last line seen", so diving into a callee and returning updated it to the callee's last line, making the return instruction on the original line look like a fresh line entry. Fixed by tracking the last-seen line per stack-frame depth instead (deeper frames' entries are invalidated on return).
- **Closures now retain private mutable state across calls** — a `make_counter`-style closure (`fn make_counter start`, returning an inner `fn counter` that does `start: start + 1`) previously reset on every call instead of persisting: assignment always wrote a fresh local instead of mutating an already-captured variable from an enclosing scope. Fixed symmetrically in the tree-walker (`Environment.Assign`, walking outward but stopping before the root/global scope) and the VM (new `OpSetFree` opcode + `SymbolTable.ResolveAssignTarget`). Sharing mutable state between two *sibling* closures over the same variable (e.g. a `{increment, get}` pair) remains a known, documented limitation of the VM (the tree-walker handles it correctly via its shared `*Environment` pointer) — a full fix needs heap-allocated upvalue cells.
- **VM: self-referential shadow reads and self-recursive local closures** — `x: x + 1` inside a nested function shadowing an outer `x` used to read the not-yet-initialized VM local as its own right-hand operand (`compileVarStatement` defined the fresh local *before* compiling its value); a self-recursive function assigned via `name: fn ...` (or `fn name ...`) and nested inside another function crashed the VM outright with a nil-pointer dereference the moment it recursed, since it captured its own name as a free variable from the enclosing scope's not-yet-initialized slot. Fixed by compiling a non-function right-hand side before defining its target (matching the tree-walker's own evaluation order), and by adding a new `OpCurrentClosure` opcode so a self-recursive function's own name resolves to the running closure at call time instead of being captured at all.
- **Lexer: a trailing comment (`code -- comment`) no longer swallows the statement-ending newline** — the mid-line comment branch consumed straight through to the next line's tokens without ever emitting the `NEWLINE` token that marks a statement boundary (unlike a bare `code\n`, handled just above it, which does), so the parser saw no boundary and kept parsing whatever followed as a continuation of the commented statement's expression (`x: 1 -- note` then `y: 2` failed with "unexpected ':'"). A common, previously undetected pattern — found building `pkg/docexamples`, which hit it constantly.
- **VM: type-mismatch errors now name the operator** — `cannot apply operator between INTEGER and STRING` became `cannot apply '+' between INTEGER and STRING`, matching the tree-walker's message. Found by `FuzzRuntimeErrors` within a second of its first run.
...
- `docs/en/16-cookbook.md`: fixed `regex_replace`'s documented argument order (real signature is `pattern, replacement, text`), replaced the phone-masking example's `(?=...)` lookahead (unsupported by Go's RE2 regex engine) with an equivalent using string slicing, and added the missing parentheses around chained calls like `print (is_email "...")` — without them, `print is_email "..."` passes two separate arguments to `print` rather than calling `is_email` first. The closure-factory examples now use the working inline `fn name: expr` form for the returned closure instead of a block-style `fn name` line, which parses as a named zero-argument declaration, not an anonymous closure. The mutable-counter example — previously called out as unsupported — now works and is shown as a real, working example, following the closure fix above.
- **`docs/en/05-functions-and-closures.md`, `docs/de/05-funktionen-und-closures.md`**: the `make_counter`/`c1` closure examples called a zero-argument closure as a bare reference (`print (counter)`), which evaluates to the function value itself without invoking it — needs the explicit `counter()` a zero-arg call requires. Fixed in both languages, with a note added on the requirement.
- **`docs/en/19-ai-builtins.md`, `docs/de/19-ki-builtins.md`**: documented the three new AI builtins above (§19.3), and updated the overview table (§19.2).
- **`pkg/docexamples` (see Added) found and fixed real bugs across 18 doc files, both languages** — not just harness false positives:
  - 66 numeric literals corrupted across 10 files (`9[0]` instead of `9.0` — also inside string literals, e.g. IP addresses and prices), apparently from some prior automated edit gone wrong; fixed with one regex pass per affected file.
  - A missing closing ` ``` ` fence in `docs/de/16-kochrezepte.md` that merged two code blocks and a heading into one.
  - ~15 missing-parens call-precedence bugs in cookbook recipes (`f g x` is `f(g, x)`, not `f(g(x))`) — several produced silently *wrong* output rather than an error.
  - A Caesar-cipher recipe whose algorithm never worked (`at` on a string returns another string, not a numeric code point — Pipe has no such conversion); rewritten to look up each character's index in the alphabet instead.
  - Multi-line map/list literals (not supported by the language) in 5 places.
  - A stray `and`/`or` instead of `&&` in a few recipes, and a `now 0` call (`now` takes no arguments).
  - Found and deliberately left unfixed: `f "text" -3` does not pass `-3` as a second argument (`-` is parsed as continuing a binary expression, not starting a fresh negative argument) — worked around in the one recipe that hit it rather than risking a parser precedence change.
