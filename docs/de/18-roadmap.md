...
## Aktuelle Version: v0.9.3.5

Pipe ist aktuell in Version **v0.9.3.5**, dem **Sandbox-Härtungs-Release** mit den Erkenntnissen aus sechs Red-Team-Audit-Runden. Es bringt einen **zentralen Egress-Gate** für alle AI-Provider-Ausgänge (macht die Fehlerklasse „vergessenes Builtin-Gate" strukturell unmöglich, inkl. Cache-Treffer), einen zentralen `checkNetworkAccess`-Helper für alle Netzwerk-Builtins (schließt auch den CLI-Flag-Pfad von `--sandbox` ab), korrekte Sandbox-Gates für `try_ai`, `embed`/`embed_batch` und `import` sowie einen Rekursions-Tiefen-Guard (E008) statt Stack-Overflow-Crashes in Tree-Walker und Bytecode-VM. Dazu kommen eine deterministische Tool-Argument-Reihenfolge, die Red-Team-Test-Suite (`examples/redteam*.pipe`) und Blog-Dokumentation aller Audit-Runden. Der Sprachumfang entspricht **v0.9.3**: ein zero-dependency MCP-Server (exponiert `ai_tool`-Funktionen als standardkonforme MCP-Tools über stdio für Claude Desktop, Cursor etc.), das X-API-v2-Modul und das Discord-Modul (Webhook + Bot-Token, beide in Entwicklung) sowie reichere CI-Benachrichtigungen mit Discord + Telegram Dual-Versand inklusive KI-Code-Reviews. Dazu kommen aus v0.9.2: `max_tokens` als optionales drittes Argument für `ai_chat`/`ai_chat_json` (Kostenkontrolle für LLM-Aufrufe), ein offizieller plattformübergreifender Installer (`install.sh`/`install.ps1`) sowie eine automatisierte GitHub Action, die den Installer mit der Website synchron hält.
...
- ✅ Bytecode Compiler + Stack-VM (42 Opcodes, 29 + 18 + 28 Tests)
