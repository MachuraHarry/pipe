...
Pipe ist aktuell in Version **v1.3.0**. Seit dem **Production-Ready-Release** v1.0.0 kamen hinzu: **Multi-Agent-Swarms** (`ai_swarm`/`ai_swarm_trace`/`ai_swarm_stream`, mit Live-Fortschrittsbeobachtung und Eingriff mitten im Lauf), **`ai_vision`**-Bildverständnis, ein 6. AI-Provider (**OpenCode Zen**, ohne API-Key nutzbar), ein **eingebauter Self-Updater** (`pipe --update`), direkter, **LLM-freier Tool-Aufruf** (`tool_call`), prozessübergreifendes **File-Locking** (`file_lock`/`file_unlock`), das **`elif`**-Schlüsselwort, das **`parallel_safe`**-Flag von `ai_tool` für parallele Tool-Batches, echte **Multi-File-`pipe -build`**-Projekte, sowie mehrere Runden Sandbox-Hardening (Audits 7-11). Es trägt weiterhin alles aus v1.0 mit: den vollständigen **MCP-Server und -Client** (stdio + SSE), **Sandbox-Profile** (deklarative Runtime-Sicherheit), die **Bytecode-VM** (bis zu 55x schneller als der Tree-Walker), **Guard Clauses** in Match-Ausdrücken, **Concurrency-Primitiven** (Channels, Mutex, Counting Semaphore) und das **MQTT-5.0-Modul**. 246 Builtins, 23 Module, 752 Tests.
