...
Runs the program or REPL using the Bytecode VM instead of the Tree-Walker interpreter. The VM is faster on function-call and recursion-heavy code — measured up to 55× on fib(20) — but simple loops and string concatenation are comparable or slower (0.6×–3×). Some features are unavailable (see Chapter 12).
