...
var builtinDocs = []BuiltinDoc{
	...
	{Name: "proc_start", Signature: "proc_start(command)", Params: []Param{p("command", "string")}, ReturnType: "number",
		Description: "Starts a command asynchronously via the shell and returns an integer process handle.", Category: CatIO},
	{Name: "proc_wait", Signature: "proc_wait(handle)", Params: []Param{p("handle", "number")}, ReturnType: "map",
		Description: "Blocks until the process finishes and returns a map with output, error, and status.", Category: CatIO},
	{Name: "proc_kill", Signature: "proc_kill(handle)", Params: []Param{p("handle", "number")}, ReturnType: "boolean",
		Description: "Terminates a running process started with proc_start.", Category: CatIO},
	{Name: "proc_running", Signature: "proc_running(handle)", Params: []Param{p("handle", "number")}, ReturnType: "boolean",
		Description: "Returns true if the process is still running, false otherwise.", Category: CatIO},
	...
	{Name: "file_lock", Signature: "file_lock(path)", Params: []Param{p("path", "string")}, ReturnType: "any",
		Description: "Blocks until an exclusive OS-level advisory lock on path is held (creating the file if needed) and returns a lock handle. Coordinates access across separate processes sharing a file with no locking of its own (e.g. two processes both reading/writing the same on-disk database file) — unlike write_file, this locks the file itself, not just a write. Released by file_unlock, or automatically if the process exits.", Category: CatFile},
	{Name: "file_unlock", Signature: "file_unlock(lock)", Params: []Param{p("lock", "any")}, ReturnType: "nil",
		Description: "Releases a lock acquired by file_lock.", Category: CatFile},
	...
	{Name: "tcp_connect_tls", Signature: "tcp_connect_tls(host, port, servername?, insecure?)", Params: []Param{p("host", "string"), p("port", "number"), p("servername", "string"), p("insecure", "boolean")}, ReturnType: "connection",
		Description: "Connects to a TLS-secured TCP server. servername defaults to host for SNI and certificate verification; insecure skips certificate verification (for self-signed certificates).", Category: CatTCP},
	...
	{Name: "tcp_read_bytes", Signature: "tcp_read_bytes(conn, n)", Params: []Param{p("conn", "connection"), p("n", "number")}, ReturnType: "bytes",
		Description: "Reads exactly n bytes from a TCP connection and returns them as bytes. Blocks until all bytes are available.", Category: CatTCP},
	{Name: "tcp_set_read_timeout", Signature: "tcp_set_read_timeout(conn, ms)", Params: []Param{p("conn", "connection"), p("ms", "number")}, ReturnType: "nil",
		Description: "Sets a read deadline of ms milliseconds on a TCP connection; 0 clears the timeout.", Category: CatTCP},
	...
	{Name: "time_ms", Signature: "time_ms()", Params: nil, ReturnType: "number",
		Description: "Returns the current time as Unix milliseconds. Use for fine-grained timing/benchmarks.", Category: CatDate},
	...
	{Name: "ai_provider", Signature: "ai_provider(name, {model, host, timeout}?)", Params: []Param{p("name", "string"), p("config", "map, optional")}, ReturnType: "string",
		Description: "Sets the AI provider: \"openai\", \"anthropic\", \"deepseek\", \"ollama\", \"openrouter\", or \"opencode\". Optionally overrides defaults with a config block, e.g. ai_provider \"deepseek\" {model: \"deepseek-v4-flash\"}.", Category: CatAIConf},
	...
	{Name: "redact", Signature: "redact(text, options?)", Params: []Param{p("text", "string"), p("options", "map, optional")}, ReturnType: "string",
		Description: "Finds and masks PII in text. AI-assisted by default (catches free-text PII like names/addresses); pass {offline: true} for a zero-network, regex-only mode (structured PII only: emails, phone numbers, IPv4, SSNs, credit-card-shaped digits).", Category: CatAIHi},
	{Name: "rerank", Signature: "rerank(query, candidates, top_k?)", Params: []Param{p("query", "string"), p("candidates", "list"), p("top_k", "int, optional")}, ReturnType: "list",
		Description: "Scores each candidate's relevance to the query (0-10) and returns them sorted descending as {text, score} maps. Completes the embed -> nearest -> rerank retrieval pipeline.", Category: CatAIHi},
	{Name: "moderate", Signature: "moderate(text)", Params: []Param{p("text", "string")}, ReturnType: "map",
		Description: "Content-safety classification against a fixed taxonomy (violence, self-harm, hate, sexual, harassment). Returns {flagged, categories, scores}. Uses OpenAI's native /v1/moderations endpoint when that's the active provider, otherwise a prompted fallback.", Category: CatAIHi},
	...
	{Name: "ai_swarm_stream", Signature: "ai_swarm_stream(task, entry_agent, max_rounds, on_progress)", Params: []Param{p("task", "string"), p("entry_agent", "string"), p("max_rounds", "number"), p("on_progress", "function")}, ReturnType: "map",
		Description: "Like ai_swarm_trace, but calls on_progress(agent, event, detail, args_json) after every swarm step so the caller can surface live status instead of waiting for the whole run to finish. event is one of \"start\" (a round begins for agent), \"tool\" (agent called the real tool named detail, with its raw JSON arguments in args_json), \"handoff\" (agent handed off to the agent named detail), or \"final\" (agent produced the answer); args_json is \"\" for every event except \"tool\". Returns {content, path, rounds} exactly like ai_swarm_trace.", Category: CatAISwarm},
	...
	{Name: "mcp_use_stdio", Signature: "mcp_use_stdio(command, args..., env?, alias?)", Params: []Param{p("command", "string"), p("args", "string")}, ReturnType: "nil",
		Description: "Connects to an external MCP server via a spawned subprocess (stdio client). The optional alias names the tool prefix (default: mcp0_, mcp1_, ...).", Category: CatMCP},
	{Name: "mcp_use_sse", Signature: "mcp_use_sse(url, alias?)", Params: []Param{p("url", "string"), p("alias", "string")}, ReturnType: "nil",
		Description: "Connects to an external MCP server over HTTP Server-Sent Events at url. The optional alias names the tool prefix (default: mcp0_, mcp1_, ...).", Category: CatMCP},
