...
const (
	...
	BuiltinScope
	FreeScope
	// FunctionScope marks a self-recursive function's own name, visible only
	// from directly inside its own body (see DefineFunctionName). It has no
	// backing stack slot or capture -- loadSymbol emits OpCurrentClosure,
	// which reads the running closure off the current frame instead.
	FunctionScope
)
...
// ResolveAssignTarget resolves the target of an assignment (`name: value`)
// for compileVarStatement, mirroring object.Environment.Assign: it reuses an
// existing binding in the current scope (identical to ResolveInCurrent), and
// otherwise walks outward through enclosing FUNCTION scopes looking for an
// already-declared binding to write through to -- letting a closure's own
// captured free variables persist mutation across calls -- but never writes
// through into the root/global scope. If no existing binding is found
// anywhere in that intermediate range, the caller must Define a fresh
// binding in the current scope, exactly as when ResolveInCurrent misses.
func (s *SymbolTable) ResolveAssignTarget(name string) (Symbol, bool) {
	// A FunctionScope hit (the enclosing recursive function's own name, see
	// DefineFunctionName) has no backing slot to write through to -- treat
	// it as not-found so the caller allocates a real local instead,
	// shadowing the synthetic self-reference rather than corrupting an
	// unrelated slot at index 0.
	if sym, ok := s.store[name]; ok && sym.Scope != FunctionScope {
		return sym, true
	}
	if s.Outer == nil || s.moduleMode {
		return Symbol{}, false
	}
	outerSym, ok := s.Outer.resolveAssignAncestor(name)
	if !ok {
		return Symbol{}, false
	}
	free := Symbol{Name: name, Scope: FreeScope, Index: len(s.FreeSymbols)}
...
// resolveAssignAncestor is the outward-walking half of ResolveAssignTarget.
// Unlike ResolveAssignTarget's own first check, it treats a GlobalScope (or
// FromImport-aliased) hit as "not found" -- so assignment can never write
// through into the root/global scope, or into a module's own aliased
// global, from a nested function; those cases always shadow with a fresh
// local instead, matching object.Environment.Assign stopping before an
// outer==nil environment.
func (s *SymbolTable) resolveAssignAncestor(name string) (Symbol, bool) {
	if sym, ok := s.store[name]; ok {
		if sym.Scope == GlobalScope || sym.FromImport || sym.Scope == FunctionScope {
			return Symbol{}, false
		}
		return sym, true
	}
	// A module's own scope is isolated from its importer (see Resolve): never
	// walk past it into the importer's ancestor chain for a write target.
	if s.moduleMode {
		return Symbol{}, false
	}
	if s.Outer == nil {
...
// DefineFunctionName binds name as the enclosing function's own name inside
// its own newly entered scope (call right after enterScope, before defining
// parameters or compiling the body). A reference to it from directly within
// the body resolves via loadSymbol to OpCurrentClosure -- the running
// closure read off the frame at CALL time -- rather than being captured as
// a Free variable from the outer scope's not-yet-initialized slot, which is
// what let a self-recursive local function (`f: fn n: ... f(n-1) ...`,
// nested inside another function) crash the VM with a nil-closure call:
// OpClosure captured the outer slot's value before the OpSetLocal that
// would have stored the completed closure into it ever ran.
...
type CompiledScope struct {
	...
	NumLocals    int
	FreeSymbols  []Symbol
	// LocalNames maps each local slot (params and body-declared locals share
	// one slot space) back to its source identifier -- debug-info only, see
	// object.CompiledFunction.LocalNames.
	LocalNames []string
}
...
type Bytecode struct {
	...
	// GlobalNames maps each global slot back to its source identifier --
	// debug-info only, mirroring CompiledScope.LocalNames at the top level.
	GlobalNames []string
}
...
func (c *Compiler) Bytecode() *Bytecode {
	...
	root := c.symbolTable.root()
	bc := &Bytecode{
		Instructions: scope.instructions,
		Lines:        scope.lines,
		Constants:    c.constants,
		SourceFile:   c.sourceFile,
		GlobalNames:  globalNamesFromTable(root),
	}
...
func (c *Compiler) Compile(node ast.Node) error {
	...
	case *ast.FnStatement:
		...
		if err := c.compileFnLiteralBody(n.Parameters, n.Body, n.Name.Value, n.Name.Value); err != nil {
			return err
		}
	...
	case *ast.FnLiteral:
		if err := c.compileFnLiteralBody(n.Parameters, n.Body, "", ""); err != nil {
			return err
		}
...
func instructionWidthAt(ins Instructions, pos int, op Opcode) int {
	...
	case OpConstant, OpGetGlobal, OpSetGlobal, OpGetLocal, OpSetLocal,
		OpGetBuiltin, OpDot, OpGetFree, OpSetFree,
...
func (c *Compiler) compileVarStatement(vs *ast.VarStatement, keepValue bool) error {
	symbol, exists := c.symbolTable.ResolveAssignTarget(vs.Name.Value)
	needsFresh := !exists || symbol.FromImport
	...
	//
	// A function-literal RHS compiles through compileFnLiteralBody with
	// vs.Name.Value as its selfName, so a self-recursive `f: fn n: ...
	// f(n-1) ...` resolves its own name from inside its own body to
	// OpCurrentClosure (see DefineFunctionName) -- never as a Free capture
	// of this not-yet-initialized slot, which is what crashed the VM for a
	// LOCAL (non-global) self-recursive function. Every other RHS is
	// compiled first and the fresh symbol defined only afterwards, matching
	// the tree-walker's own evaluation order (env.Eval(vs.Value) before
	// env.Assign) -- otherwise a self-shadowing read like `x: x + 1`, where
	// x is not yet bound in this scope, would resolve against its own
	// not-yet-initialized slot instead of whatever `x` already existed
	// outside.
	if fnLit, isFnLiteral := vs.Value.(*ast.FnLiteral); needsFresh && isFnLiteral {
		symbol = c.symbolTable.Define(vs.Name.Value)
		needsFresh = false
		if err := c.compileFnLiteralBody(fnLit.Parameters, fnLit.Body, vs.Name.Value, vs.Name.Value); err != nil {
			return err
		}
	} else if fnLit, isFnLiteral := vs.Value.(*ast.FnLiteral); isFnLiteral {
		// needsFresh is false here -- typically a top-level `name: fn ...`,
		// where defineTopLevelSymbols' forward-reference pre-pass already
		// Define()'d `name` before this statement compiles, so the
		// self-recursion-enabling branch above never fires (global
		// self-reference already resolves correctly via plain OpGetGlobal,
		// see Resolve()'s GlobalScope special case). Still pass vs.Name.Value
		// as the debug NAME (unrelated to self-reference) so the function
		// isn't shown as anonymous in a stack trace.
		if err := c.compileFnLiteralBody(fnLit.Parameters, fnLit.Body, "", vs.Name.Value); err != nil {
			return err
		}
	} else {
	...
	case FreeScope:
		// The name resolved to an already-captured free variable (see
		// ResolveAssignTarget): write through to the closure's own copy so
		// mutations persist across calls to this same closure instance.
...
// compileFnLiteralBody compiles a function's parameters and body into a new
// scope, emitting the resulting OpClosure (with its captured free
// variables) into the CURRENT (enclosing) scope. Shared by *ast.FnStatement
// and *ast.FnLiteral (including a VarStatement's `name: fn ...` form, via
// compileVarStatement).
//
// selfRefName, when non-empty, makes the function's own name resolve to
// OpCurrentClosure from directly inside its own body (see
// SymbolTable.DefineFunctionName) instead of being captured as a Free
// variable from the enclosing scope's slot -- which, for a LOCAL (non-
// global) self-recursive function, would still be uninitialized at the
// moment the closure captures it (the capture happens before the
// enclosing OpSetLocal that stores the finished closure ever runs),
// producing a nil self-reference that crashes the VM the first time the
// function recurses. Only pass this when the caller has ALREADY allocated
// (or is about to allocate) selfRefName's own binding slot in the
// enclosing scope before this call, i.e. the function's own name really
// does already resolve to it -- passing a name that isn't yet a real
// binding would make DefineFunctionName's synthetic entry shadow a
// genuinely different, not-yet-bound outer resolution.
//
// debugName is independent of selfRefName -- it's the function's display
// name for debugger stack traces (object.CompiledFunction.Name), set
// whenever the caller happens to know one (usually vs.Name.Value or
// n.Name.Value at the call site) regardless of whether self-reference is
// enabled. Pass "" for a genuinely anonymous function literal.
func (c *Compiler) compileFnLiteralBody(params []*ast.Identifier, body *ast.BlockStatement, selfRefName, debugName string) error {
	c.enterScope()
	if selfRefName != "" {
		c.symbolTable.DefineFunctionName(selfRefName)
	}
	for _, p := range params {
		c.symbolTable.Define(p.Value)
	}

	if err := c.compileFnBody(body); err != nil {
		return err
	}
	...
	idx := c.addConstant(&object.CompiledFunction{
		...
		Name:         debugName,
		LocalNames:   compiledFn.LocalNames,
	})
...
func (c *Compiler) leaveScope() CompiledScope {
	...
	localNames := localNamesFromTable(c.symbolTable, nl)
	c.scopes = c.scopes[:c.scopeIndex]
	c.scopeIndex--
	c.symbolTable = c.symbolTable.Outer
	return CompiledScope{Instructions: scope.instructions, Lines: scope.lines, NumLocals: nl, FreeSymbols: free, LocalNames: localNames}
}

// localNamesFromTable builds a slot-index -> source-name table for every
// LocalScope symbol in t (params and body-declared locals share one slot
// space, both Define()'d the same way). FunctionScope entries (a self-
// recursive function's own name, resolved via OpCurrentClosure -- see
// SymbolTable.DefineFunctionName) have no backing slot and are skipped.
// Debug-info only; never consulted by the VM's own execution.
func localNamesFromTable(t *SymbolTable, numLocals int) []string {
...
// globalNamesFromTable is localNamesFromTable's GlobalScope counterpart,
// for the root symbol table's own slot space.
func globalNamesFromTable(root *SymbolTable) []string {
	names := make([]string, root.numDefinitions)
	for name, sym := range root.store {
		if sym.Scope == GlobalScope && sym.Index >= 0 && sym.Index < len(names) {
			names[sym.Index] = name
		}
	}
	return names
